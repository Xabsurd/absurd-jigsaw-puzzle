...
export default class UserControl {
  ...
  public on(
    eventName: UserControlEventName,
    cb: UserControlEvent[UserControlEventName]
  ) {
  ...
  private wheel(event: FederatedWheelEvent) {
    ...
    const opposeX =
      (event.screenX - this.mainContainer.x) / this.mainContainer.scale.x;
    const opposeY =
      (event.screenY - this.mainContainer.y) / this.mainContainer.scale.x;
    const scale = Math.max(
      this.minScale,
      Math.min(
        this.maxScale,
        this.mainContainer.scale.x * (1 - event.deltaY / 1000)
      )
    );
