...
export default class UserControl {
  ...
  public handle(target: Container) {
    ...
    target.on('pointerdown', this.pointerdown, this);
  }

  public dehandle(target: Container) {
    target.off('pointerdown', this.pointerdown, this);
  }
  ...
  private pointerdown(event: FederatedPointerEvent) {
    ...
    this.mainContainer.addChild(target);
    target.cursor = 'grabbing';
    target.alpha = 0.92;
    this.#dragTarget = {
      cachePosition: { x: target.x, y: target.y },
      mousePosition: { x: event.global.x, y: event.global.y },
      target,
      needScale: true
    };
    this.listener.get('pointerdown')?.(target);
  }
  ...
  private pointerup(event?: FederatedPointerEvent) {
    ...
    if (!target.destroyed) {
      target.alpha = 1;
      if (target !== this.mainContainer) target.cursor = 'grab';
    }
