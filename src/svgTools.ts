export class GeneratePath {
  ...
  getPath() {
    return { dh: this.gen_dh().str, dv: this.gen_dv().str };
  }
...
export class TileTool {
  ...
  getTilePath(x: number, y: number) {
    ...
    if (x < this.data.dv.lines.length) {
      ...
      const prev_dh_path = this.data.dh.lines[y - 1]?.paths[x]
        .trim()
        .split(" ") || [this.data.width, "0"];
      const dh_path = this.data.dh.lines[x]?.paths[y].trim().split(" ") || [
        this.data.width,
        "0",
      ];
      line3 = {
        start:
          x < this.data.dh.lines.length
            ? "M " +
              dh_path[dh_path.length - 2] +
              " " +
              dh_path[dh_path.length - 1]
    ...
    if (y < this.data.dh.lines.length) {
      ...
      line2 = {
        start:
          y > 0 ? this.data.dv.lines[y - 1].start : `M 0 ${this.data.height}`,
  ...
  reverseDV(x: number, y: number) {
    const dv = this.data.dv.lines[x].paths[y];
    const prev_dv =
      y > 0 ? this.data.dv.lines[x].paths[y - 1] : this.data.dv.lines[x].start;
    const prev_dv_str = prev_dv.trim().split(" ");
  ...
  reverseDH(x: number, y: number) {
    const dh = this.data.dh.lines[y].paths[x];
    const prev_dh =
      x > 0 ? this.data.dh.lines[y].paths[x - 1] : this.data.dh.lines[y].start;
    const prev_dh_str = prev_dh.trim().split(" ");
